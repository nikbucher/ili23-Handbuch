ifndef::env-github[:toc: left]
:toc-title: Inhalt
:toclevels: 5
:figure-caption: Abbildung
:example-caption!:
:table-caption: Tabelle
:note-caption: Hinweis
:icons: font
:data-uri:
:source-highlighter: highlight.js
:sectnums:
:sectnumlevels: 5
:sectlinks:
:sectanchors:
:xrefstyle: full
:chapter-refsig: Kapitel
:section-refsig: Kapitel

= Modellierung raumbezogener Daten: Eine Einführung unter Berücksichtigung von UML und INTERLIS
Joseph Dorfschmid, Sascha Brawer
:revdate: 28.07.2009
:revnumber: 1.0
:title-page:

[.text-right]
🌐 Sprache: *Deutsch* | link:../fr/[Français] | link:../en/[English] | link:../[⌂]

// Die Verweise auf die Inhalte der Dokumentation sollen im README auf Github nicht angezeigt werden.
ifndef::env-github[]

include::00.adoc[]

include::01.adoc[]
include::02.adoc[]
include::03.adoc[]
include::04.adoc[]
include::05.adoc[]
include::06.adoc[]
include::07.adoc[]
include::08.adoc[]
include::09.adoc[]
include::10.adoc[]
include::11.adoc[]

endif::[]

// Erklärung und Link auf Website
ifdef::env-github[]
In diesem Repository wird die Online-Version des INTERLIS 2.3 Benutzerhandbuches gepflegt. Ergänzungen und Korrekturen bitte als Issue oder allenfalls PR erfassen.

Die aktuelle Website ist unter https://nikbucher.github.io/ili23-handbuch/de/ erreichbar.
endif::[]
